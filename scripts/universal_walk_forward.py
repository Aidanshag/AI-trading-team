...
def evaluate_cell(strategy_fn, bars: pd.DataFrame, symbol: str) -> dict:
    """One strategy × symbol × session run. Returns per-side stats."""
    if len(bars) < 200:
    ...
    for side in ("long", "short"):
        ...
        per_side[side] = {
            "all": compute_stats([t.r_multiple for t in trades]),
            "oos": compute_stats([t.r_multiple for t in oos_trades]),
        }
    return {
        "per_side": per_side,
        "bar_count": len(bars),
        ...
        for session in sessions:
            ...
            for strat_name in strategy_names:
                ...
                for side in ("long", "short"):
                    ...
                    eligible = (
                        oos["n"] >= args.min_n
                        and oos["t"] is not None and oos["t"] >= args.min_t
                        and oos["E"] is not None and oos["E"] >= args.min_E
                    )
                    cells.append({
                        "strategy": strat_name,
                        "symbol": sym,
