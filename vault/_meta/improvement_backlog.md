...
- [P0] [effort: 90min] [risk: medium] [status: merged 2026-05-15 ab5063b (call-site migration landed earlier at 326ab7f)] [autonomous-eligible: yes]
  **Migrate profit-lock from `place_order(order_type="limit")` to `close_position`** — SHIPPED. The two flatten call sites (target_hit + trailing_lock) were migrated to `client.close_position` at commit `326ab7f` (2026-05-14 01:02). Loose-end work completed at commit `ab5063b` (2026-05-15): added `_record_close_decision()` helper that writes a `decisions` row (agent='profit_lock', kind='close') on each profit-lock fire — restoring the audit trail that the orders-table row used to provide; added two regression-guard tests (`test_profit_lock_close_uses_close_position_not_limit_order`, `test_profit_lock_close_writes_decisions_row`). 476 tests passing. The stop-replace path in `_trail_broker_stop_to_floor` still uses `place_order(order_type="stop_limit")` — verified correct (cancel+replace of a STOP, not a flatten; broker accepts).
  Historical: 13 `profitlock_*` LIMIT orders REJECTED by ProjectX over 2026-05-13/14, plus 1 `emergency_flat_*` LIMIT rejected — broker quirk documented in `project_broker_order_semantics.md` (memory). Net effect of the original bug: when the mechanical profit-lock TRIED to fire, it FAILED. Trades retraced or stopped out instead.
...
- [P0] [effort: 180min] [risk: low] [status: merged 2026-05-15] [autonomous-eligible: yes]
  **Sentinel — continuous autonomous anomaly watcher** — SHIPPED 2026-05-15. `tools/sentinel.py` with 6 invariant checks: mock_orders_in_db / open_position_economics / close_slippage_vs_floor / orphan_working_orders / brain_vs_trader_rate / duplicate_trader_procs. Each finding posts to Discord with severity, daily report appended to `vault/_meta/sentinel_YYYY-MM-DD.md`. 18 new tests cover every check with injectable mocks. 519 tests green. Smoke test on live state: 0 findings (clean). Standalone: `python -m tools.sentinel [--dry] [--report]`.
  Scheduled task `FundSentinel` REGISTERED 2026-05-15 (every 10 min, idempotent installer at `scripts/install-sentinel-daily.ps1`). Verified State=Ready. First run already fired.
...
- [P0] [effort: 180min] [risk: medium] [status: already-shipped 2026-05-14 326ab7f / verified 2026-05-15] [autonomous-eligible: yes]
  **Trailing broker stop — replace software-polled profit-lock with broker-side trailing stop** — ALREADY SHIPPED 2026-05-14 in commit 326ab7f. `_trail_broker_stop_to_floor()` in `tools/profit_protect.py` (line 390) runs every check_and_close poll: when peak crosses each tier, the broker-side stop is moved UP via cancel+replace (modify_order was silently ineffective per memory `project_broker_order_semantics`). Process-local memoization prevents duplicate trails to the same floor. Verified 2026-05-15 via code inspection. Software-polled close path stays as belt-and-suspenders. Continuous percent-of-peak retracement (cycle 3 fdafe72) makes the floor tighter than the old tier table — addresses the user's "$100→$30" concern directly.
...
- [P1] [effort: 120-180min] [risk: low] [status: already-shipped — graduated by FundValidateLiveFilter daily pipeline] [autonomous-eligible: yes]
  **Walk-forward validation of MGC cells** — ALREADY DONE. As of 2026-05-15 audit, all 4 MGC cells in `live_allowlist` have `experimental: False` with validated OOS stats: narrow_range_break|Asian|long (n=47 t=1.986), narrow_range_break|Asian|short (n=35 t=2.031), fair_value_gap_tuned|Asian|short (n=30 t=1.953), inside_bar_break|PostClose|long (n=25 t=1.52). All meet promotion criteria (n≥25, t≥1.5). The daily FundValidateLiveFilter scheduled task (06:00 ET) filled the OOS stats and graduated the cells out of shadow. No action required.
